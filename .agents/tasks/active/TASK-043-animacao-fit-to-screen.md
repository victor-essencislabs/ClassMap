...
- [x] CA-01: clicar em "Ajustar à tela" no Diagrama de Classes assenta suavemente (~150ms), nos dois temas. Verificado por leitura de código + teste automatizado do ciclo `settling`; **sem confirmação visual ao vivo** (ver "Divergências" — harness de preview deste ambiente serve o checkout do repositório principal, não este worktree isolado).
- [x] CA-02: mesmo comportamento no Diagrama de Objetos. Mesma implementação/hook compartilhado (`useCanvasZoomPan`) e mesma classe `settling` aplicada no wrapper de `ObjectDiagramCanvas.tsx` — mesma ressalva de validação visual do CA-01.
...
Nenhum handoff pendente para continuar a implementação — código, testes automatizados e documentação da task estão completos e commitados nesta branch (`task/043-animacao-fit-to-screen`). Pendência real é só a validação visual manual (ver "Pendências" acima), que depende de um ambiente com o app rodando a partir do código desta branch (ou já mesclada) — não é um handoff de trabalho inacabado.
