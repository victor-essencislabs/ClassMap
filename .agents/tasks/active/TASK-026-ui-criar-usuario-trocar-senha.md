...
### Achados e correções pós-deploy (2026-08-31, validação ao vivo com o usuário)
- **Bug real, não introduzido por esta task**: a migration da RPC `find_user_id_by_email` (TASK-012/ADR-004) nunca tinha sido aplicada ao Supabase de produção — só existia validada localmente. Isso só quebrou na prática agora porque esta é a primeira vez que existe um segundo usuário real para exercitar "Adicionar por e-mail" contra produção. Corrigido aplicando a migration faltante via SQL Editor do painel (ver `supabase/README.md`). Sem isso, adicionar um usuário já existente a um projeto retornava `404` mesmo com todo o resto correto.
- **Feedback de UX do usuário, corrigido na mesma sessão**: (1) lista de membros mostrava só nome/id truncado, sem e-mail, dificultando identificar quem é quem — `TASK-027` (`list_organization_members_with_email`/`list_project_members_with_email`, novas RPCs `SECURITY DEFINER`) resolve isso, e2 `AccessMember`/`OrganizationMember`/`ProjectMember` ganharam `email: string`; (2) o formulário de adicionar (nos dois modos) estava com o select de papel desalinhado do campo de e-mail, e o botão de submit flutuando fora do lugar quando a legenda de papel/aviso de vínculo de organização mudava a altura do card — `.access-add-form` trocou de `flex` com `align-items:flex-end` para `display:grid` de 2 colunas, com hints/avisos em linha própria (`.field-full`) e botão de submit também em linha própria (`.access-add-submit`), imune à altura variável dos vizinhos.
