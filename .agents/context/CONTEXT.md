...
6. **Objetos não podem ser ligados entre si no Diagrama de Objetos** (feedback de 2026-08-29, revisão da tela publicada — comportamento era deliberado desde a TASK-008, mas o usuário pediu explicitamente para habilitar) → `ADR-006` (link simples entre instâncias, sem os 5 tipos UML/multiplicidade — esses descrevem relação entre classes, não entre objetos concretos; rejeitou paridade visual total com o Diagrama de Classes por inconsistência semântica) → `TASK-017` (`frontend-diagramas`) — **ainda em `backlog/`**.

### Onda 1 de backlog via subagentes paralelos (2026-08-29)

Pedido do usuário: paralelizar o desenvolvimento do backlog (TASK-011..017) com subagentes, sem conflito. As 5 tasks sem dependência entre si (TASK-011, 012, 014, 015, 016) foram despachadas simultaneamente, cada uma num worktree git isolado e branch própria (`task/0NN-slug`), sem push. Cada agente rodou `npm install`/`build`/`lint`/`test` no próprio worktree, preencheu "Registro de execução"/"Validação" na task e moveu o arquivo de `backlog/` para `active/` — nenhum tocou `CONTEXT.md`/`decisions/README.md`, para não gerar conflito entre si. Depois, cada branch foi mesclada sequencialmente em `main` nesta sessão, com `build`/`lint`/`test` revalidados a cada merge (usando `vitest run --exclude "**/.claude/worktrees/**"` para não contar os worktrees dos agentes ainda ativos como parte da suíte). Um único conflito real ocorreu, previsto de antemão (TASK-014 e TASK-015 editaram a mesma região de `ClassDiagramCanvas.tsx` — cor do dot vs. `<span className="name">`) e foi resolvido preservando as duas mudanças. Resultado: 111 testes passando, build/lint limpos, nenhuma das 5 tasks movida para `completed/` (todas com pendência de validação manual contra um Supabase real, mesmo padrão já registrado para TASK-001..005). Nenhum commit foi enviado ao repositório remoto — decisão de dar push fica com o usuário. TASK-013 e TASK-017 (que dependiam de TASK-012 e TASK-015 respectivamente terminarem primeiro, por tocarem os mesmos arquivos) formam a onda 2, despachada em seguida pelo mesmo mecanismo.
